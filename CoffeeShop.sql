...
/* =========================================================
   INVOICE HISTORY (Thay thế hoàn toàn ProductPriceHistory)
   Thiết kế khớp chính xác với các cột trong giao diện hình ảnh
   ========================================================= */
CREATE TABLE InvoiceHistory (
    InvoiceHistoryId INT IDENTITY(1,1) NOT NULL,
    
    -- Các cột thông tin hóa đơn chung (Phần bên trái Grid)
    MaHD INT NOT NULL,                  -- Khớp cột "Mã HD"
    NgayTao DATETIME2(0) NOT NULL,       -- Khớp cột "Ngày tạo"
    NhanVienBan NVARCHAR(100),           -- Khớp cột "Nhân viên bán"
    Ban NVARCHAR(50),                    -- Khớp cột "Bàn"
    TongTien DECIMAL(18,2),              -- Khớp cột "Tổng Tiền"
    
    -- Các cột thông tin sản phẩm (Phần bên phải Grid)
    ProductId INT NOT NULL,              -- Khớp cột "ProductId"
    ProductName NVARCHAR(150),           -- Khớp cột "ProductN"
    CategoryId INT,                      -- Khớp cột "Category"
    CategoryName NVARCHAR(100),          -- Khớp cột "CategoryN"
    Price DECIMAL(18,2),                 -- Khớp cột "Price"
    [Description] NVARCHAR(500),         -- Khớp cột "Descriptio"
    Unit NVARCHAR(50),                   -- Khớp cột "Unit"
    [Image] NVARCHAR(255),               -- Khớp cột "Image"
    [Status] NVARCHAR(20),               -- Khớp cột "Status"
    CreatedAtA DATETIME2(0),             -- Khớp cột "CreatedAtA"
    UpdatedAtA DATETIME2(0),             -- Khớp cột "UpdatedAtA"
    IsActive BIT,                        -- Khớp cột "IsActive"

    CONSTRAINT PK_InvoiceHistory PRIMARY KEY (InvoiceHistoryId)
);
GO
...
/* =========================================================
   4. SEED DATA (Thay thế phần INSERT của ProductPriceHistory)
   Dữ liệu mẫu khớp với các dòng hiển thị trong hình ảnh
   ========================================================= */
INSERT INTO InvoiceHistory 
(MaHD, NgayTao, NhanVienBan, Ban, TongTien, ProductId, ProductName, CategoryId, CategoryName, Price, [Description], Unit, [Image], [Status], CreatedAtA, IsActive)
VALUES 
    -- Dòng 1 trong hình
    (1, '2026-04-01 08:00:00', N'Admin', N'T01', 42000, 11, N'Bánh tiramisu', 5, N'Bánh ngọt', 42000.00, N'Bánh tiramisu mềm mịn', N'Phần', N'images\\banh-tiramisu.jpg', N'Available', '2026-03-01 09:00:00', 1),
    
    -- Dòng 2 trong hình
    (1, '2026-04-01 08:00:00', N'Admin', N'T01', 72000, 12, N'Croissant bơ', 5, N'Bánh ngọt', 30000.00, N'Bánh sừng bò bơ', N'Phần', N'images\\croissant-bo.jpg', N'Available', '2026-03-01 09:05:00', 1),
    
    -- Dòng 3 trong hình
    (2, '2026-04-02 10:00:00', N'Staff', N'T04', 36000, 14, N'Americano', 1, N'Cà phê', 36000.00, N'Cà phê americano', N'Ly', N'images\\americano.jpg', N'Available', '2026-03-01 09:15:00', 1);
GO
